...
fn test_sequence_example_parse_and_render() {
    ...
    assert_eq!(diagram.messages.len(), 6);
    assert_eq!(diagram.notes.len(), 2);
    assert_eq!(diagram.fragments.len(), 3);
    ...
    assert!(svg.contains("loop") || svg.contains("retry"));
    let roundtrip = diagram::sequence::parse(&diagram.to_mermaid()).unwrap();
    assert_eq!(roundtrip.messages.len(), 6);
    assert_eq!(roundtrip.notes.len(), 2);
    assert_eq!(roundtrip.fragments.len(), 3);
}
